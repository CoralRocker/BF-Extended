...
optList = open("./opts.txt", "r").readlines()
...
times =  list(map(lambda opt: rxp2.search(open(opt[0]+".dat","r").readlines()[-1]).group(1), optList))
maxSize = [maxSize]+[max(map(len,list(map(str,times))))]


for index, optTup in enumerate(optList):
    line  = rxp2.search(open(optTup[0]+".dat", "r").readlines()[-1]).group(1)
